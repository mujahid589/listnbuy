...
class CmsSettingController extends Controller
{
    ...
    public function updateHome(Request $request)
    {
        ...
        if($request->hasFile('vehicles_header')){
            $rules['vehicles_header'] =  ['mimes:png,jpg,jpeg','max:1024'];
        }
        if($request->hasFile('motorbikes_header')){
            $rules['motorbikes_header'] =  ['mimes:png,jpg,jpeg','max:1024'];
        }
        if($request->hasFile('auto_parts_header')){
            $rules['auto_parts_header'] =  ['mimes:png,jpg,jpeg','max:1024'];
        }
        if($request->hasFile('general_header')){
            $rules['general_header'] =  ['mimes:png,jpg,jpeg','max:1024'];
        }
        if($request->hasFile('handy_man_header')){
            $rules['handy_man_header'] =  ['mimes:png,jpg,jpeg','max:1024'];
        }
        if($request->hasFile('junk_cars_header')){
            $rules['junk_cars_header'] =  ['mimes:png,jpg,jpeg','max:1024'];
        }
        if($request->hasFile('local_rent_header')){
            $rules['local_rent_header'] =  ['mimes:png,jpg,jpeg','max:1024'];
        }
        if($request->hasFile('coming_soon_header')){
            $rules['coming_soon_header'] =  ['mimes:png,jpg,jpeg','max:1024'];
        }
        ...
        if($request->hasFile('vehicles_header')){
            deleteImage($cms->vehicles_header);
            $data['vehicles_header'] =  uploadImage($request->vehicles_header, 'banners');
        }
        
        if($request->hasFile('motorbikes_header')){
            deleteImage($cms->motorbikes_header);
            $data['motorbikes_header'] =  uploadImage($request->motorbikes_header, 'banners');
        }

        if($request->hasFile('auto_parts_header')){
            deleteImage($cms->auto_parts_header);
            $data['auto_parts_header'] =  uploadImage($request->auto_parts_header, 'banners');
        }

        if($request->hasFile('general_header')){
            deleteImage($cms->general_header);
            $data['general_header'] =  uploadImage($request->general_header, 'banners');
        }

        if($request->hasFile('handy_man_header')){
            deleteImage($cms->handy_man_header);
            $data['handy_man_header'] =  uploadImage($request->handy_man_header, 'banners');
        }

        if($request->hasFile('junk_cars_header')){
            deleteImage($cms->junk_cars_header);
            $data['junk_cars_header'] =  uploadImage($request->junk_cars_header, 'banners');
        }

        if($request->hasFile('local_rent_header')){
            deleteImage($cms->local_rent_header);
            $data['local_rent_header'] =  uploadImage($request->local_rent_header, 'banners');
        }
        if($request->hasFile('coming_soon_header')){
            deleteImage($cms->coming_soon_header);
            $data['coming_soon_header'] =  uploadImage($request->coming_soon_header, 'banners');
        }

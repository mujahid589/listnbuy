...
class FrontendController extends Controller
{
    ...
    public function ourteam(){
        $ourteams = OurTeam::where('type','director')->orderby('order','ASC')->get();
     ...
     * @return void
     */
    public function userReviewPage(Ad $ad)
    {
        // return $ad;
        $ad->increment('total_views');
        $ad = $ad->load(['customer', 'brand', 'adFeatures', 'galleries', 'town', 'city']);
        ...
        if ($ad->status === 'expired' && $ad->customer->id !== auth('customer')->id()) {
            ...
            return view('frontend.seller-profile', compact('ad', 'lists'));
        }
    }
    ...
    {
        // dd($request);
        $query = Ad::where('title', 'LIKE', "%$request->keyword%")->where('status', '!=', 'expired')->get();
        ...
        // $data['adMaxPrice'] = $price = \DB::table('ads')->max('price');
        // dd($data);
        return view('frontend.search-ad-list', $data);
    }
